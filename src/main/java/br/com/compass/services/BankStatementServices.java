...
public class BankStatementServices {
    ...
    public static List<Transactions> bankStatement(String cpf) {
        ...
        try{
            String query = "SELECT a FROM Transactions a WHERE a.account.cpf = :cpf ORDER BY a.transactionDate DESC";
            ...
            for(Transactions a : transactionsList){
                System.out.printf("\n%s - %s: R$ %.2f", a.getTransactionDate().format(formatter), a.getTransactionType(), a.getAmount());
            }
